...
function newServiceConnect(service, pattern) {
  ...
  options.path = '/' + querystring.escape(service) + '/' + pattern + '/';
  options.method = 'POST';
  ...
  var sock = settings.http.request(options, function(res) {
	...
	sock.on('error', function(e) {
		console.log('problem with socket: ' + e.message);
		services[i].self.emit('error', e);
		// try reconnecting...
	});	
	
	// NOTE: must do either .write() OR .end() to get bi-directional com working.
